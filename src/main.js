...
app.ws.use(
  ...
  ctx.websocket.on('message', async(data) => {
    ...
    try{
      ...
      //만일 예외가 발생하지 않으면 catch문 실행기회를 갖지 않는다.
    ...
    ***************************/
    /*
    문제제기 - 현재는 메시지를 보낸 사람에게만 돌려주고 있는 유니캐스트이다.
    만일 모든 사람에게 메시지를 보내고 싶다면 어떻게해야 할까?
    브로드캐스트 처리를 하면 된다.
    */
    //서버에 접속한 여러 클라이언트에 대한 접점(소켓 - 서버에 있지만 Client소켓)
    //서버에 있는 소켓이지만 클라이언트 소켓이므로 청취한 메세지를 쓸 수 있다.(말할 수 있다.)
    const {server} = app.ws
    //null에 대한 체크를 한다. - 안정성
    //server가 널이면 속성이나 함수를 호출할 수 없다.
    if(!server){ //앞에 not이 있다.
      return // return을 만나면 use 함수 전체를 탈출함
    }
    //clients - 소켓 여러개
    //client - 한 개 소켓
